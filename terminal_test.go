...
func drainPTY(t *testing.T, mfd int, waitCh <-chan error, kill func(), timeout time.Duration) []byte {
	return drainPTYKeys(t, mfd, waitCh, kill, timeout, false)
}

// drainPTYKeys optionally presses "q" periodically — interactive
// completions land on the Results screen and wait for the operator, so
// tests act as one. Harmless mid-run (q = navigate to Dashboard).
func drainPTYKeys(t *testing.T, mfd int, waitCh <-chan error, kill func(), timeout time.Duration, pressQ bool) []byte {
	t.Helper()
	...
	for {
		if pressQ && !finished && time.Since(lastQ) > 1200*time.Millisecond {
			_, _ = unix.Write(mfd, []byte("q"))
			lastQ = time.Now()
		}
...
func runOnPTY(t *testing.T, home string, poison bool, bin string, args ...string) (out []byte, healed bool) {
	return runOnPTYQ(t, home, poison, false, bin, args...)
}

func runOnPTYQ(t *testing.T, home string, poison, pressQ bool, bin string, args ...string) (out []byte, healed bool) {
	t.Helper()
	mfd, s := openPTY(t)
	...
	out = drainPTYKeys(t, mfd, waitCh, func() { _ = cmd.Process.Kill() }, 120*time.Second, pressQ)
...
func TestTerminalRunTUIPoisonedPTYAndRepeat(t *testing.T) {
	...
	// handed back healed.
	out, healed := runOnPTYQ(t, home, true, true, bin,
		"run", csv, "--stale-timeout", "0", "--retry-delay", "1")
	if !bytes.Contains(out, []byte("\x1b[?1049h")) {
		t.Fatalf("TUI never started (no alternate screen):\n%s", out)
	}
	if !bytes.Contains(out, []byte("MIGRATION COMPLETE")) {
		t.Fatalf("completion must land on the Results screen:\n%s", out)
	}
	// After Q on the Results screen: short confirmation only — the full
	// report must never be dumped a second time into the shell.
	post := out[bytes.LastIndex(out, []byte("\x1b[?1049l")):]
	if !bytes.Contains(post, []byte("complete:")) || !bytes.Contains(post, []byte("Results: ")) {
		t.Fatalf("short exit confirmation missing after TUI:\n%s", post)
	}
	if bytes.Contains(post, []byte("Total runtime")) || bytes.Contains(post, []byte("Failed Messages (")) {
		t.Fatalf("full report dumped after the Results screen (duplicate):\n%s", post)
	}
	...
	out2, healed2 := runOnPTYQ(t, home, true, true, bin,
		"run", csv, "--stale-timeout", "0", "--retry-delay", "1")
	if !bytes.Contains(out2, []byte("NOTHING NEW TO COPY")) {
		t.Fatalf("instant re-run must show the nothing-new verdict:\n%s", out2)
	}
...
// TestTerminalSIGHUPRestoresAndStops: a hangup mid-transfer must never
// leave the terminal raw (the historic poison source — present in every
// release before v2.0.3). Expect a graceful interrupted exit and a fully
// sane tty.
func TestTerminalSIGHUPRestoresAndStops(t *testing.T) {
	bin := buildBinary(t)
	dir, home := t.TempDir(), t.TempDir()
	src := fakeimap.NewServer(acct("hank", 80))
	dst := fakeimap.NewServer(fakeimap.NewAccount("hank2", "pw"))
	dst.AppendDelayMS.Store(80) // slow enough that the signal lands mid-run
	if err := src.Start(); err != nil {
		t.Fatal(err)
	}
	if err := dst.Start(); err != nil {
	...
	defer src.Stop()
	defer dst.Stop()
	csv := csvFor(t, dir, "h.csv", src.Port(), dst.Port(), [][2]string{{"hank", "hank2"}})

	mfd, s := openPTY(t)
	defer unix.Close(mfd)
	cmd := exec.Command(bin, "run", csv, "--stale-timeout", "0", "--retry-delay", "1")
	cmd.Stdin, cmd.Stdout, cmd.Stderr = s, s, s
	cmd.SysProcAttr = &syscall.SysProcAttr{Setsid: true}
	cmd.Env = append(os.Environ(), "HOME="+home, "TERM=xterm-256color",
		"XDG_CONFIG_HOME="+home+"/.config", "XDG_STATE_HOME="+home+"/.local/state",
	...
	waitCh := make(chan error, 1)
	go func() { waitCh <- cmd.Wait() }()
	time.Sleep(2 * time.Second) // mid-transfer
	if err := cmd.Process.Signal(syscall.SIGHUP); err != nil {
		t.Fatal(err)
	}
	out := drainPTY(t, mfd, waitCh, func() { _ = cmd.Process.Kill() }, 60*time.Second)
	if state := cmd.ProcessState; state == nil || state.ExitCode() != 130 {
		t.Fatalf("SIGHUP must exit as interrupted (130), got %v\n%s", cmd.ProcessState, out)
	}
	tio, err := unix.IoctlGetTermios(int(s.Fd()), unix.TCGETS)
	if err != nil {
		t.Fatal(err)
	...
	for name, on := range map[string]bool{
		...
		if !on {
			t.Fatalf("SIGHUP left the terminal without %s\n%s", name, out)
		}
	}
	s.Close()
}
