...
from .report import Session, logger_factory, print_summary, write_results_csv
from .sysmon import SysMon
from .util import fmt_bytes, fmt_dhms

COMMANDS = {"run", "init", "check", "import-state", "capabilities", "verify", "compact"}
...
def _cfg_from(args) -> RunConfig:
    ...
    return RunConfig(
        ...
        trace=args.trace, debug=args.debug,
...
def cmd_run(args) -> int:
    ...
    async def root():
        ...
        return await run_migration(cfg, specs, stats, session, logger_factory(logs), stop_event)
    ...
    def on_signal():
        ...
        if interrupted["n"] == 1 and "stop" in holder:
            holder["stop"].set()
            session.log("interrupt received (Ctrl+C / SIGTERM) — graceful stop, state is safe")
        else:
            task.cancel()
    ...
    session.log(f"=== {PRODUCT} v{__version__} run {cfg.run_id} end: runtime={fmt_dhms(runtime)} "
                f"ok={counts.get('SUCCESS', 0)} partial={counts.get('PARTIAL', 0)} "
